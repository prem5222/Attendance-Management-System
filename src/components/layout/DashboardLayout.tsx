  ...
  return (
    <div className="min-h-screen bg-black/70 backdrop-blur-sm flex">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <>
            ...
            >
              <Sidebar items={DASHBOARD_NAV} mobile onClose={() => setMobileMenuOpen(false)} />
            </motion.div>
          </>
        )}
      </AnimatePresence>
      ...
      <div className="flex-1 flex flex-col lg:ml-64 min-w-0 transition-all duration-300">
        {/* Mobile Header */}
        <div className="lg:hidden sticky top-0 z-30 flex items-center px-4 h-16 bg-black/80 backdrop-blur-xl border-b border-white/10">
          <button
          ...
          <span className="ml-3 font-semibold text-lg text-white">Dashboard</span>
        </div>
